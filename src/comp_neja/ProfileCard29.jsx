...
const ProfileCard29 = ({
  ...
  const imageOffset = -Math.min(scrollY * 0.15, 80);
    const progress = Math.min(scrollY / maxScroll, 1);
    const sliderOffset = Math.pow(progress, 0.85) * 60;
  ...
  style={{
    height: `${Math.max(65, 100 - scrollY * 0.05)}vh`
  }}
>
    ...
    transform: `translate3d(0, ${imageOffset}px, 0)`
        ...
        <div className="absolute right-3 top-28 h-20 w-2 bg-white/20 rounded-full z-10">
          <div
  className="absolute left-0 w-full h-12 bg-white rounded-full"
  style={{
    transform: `translate3d(0, ${sliderOffset}px, 0)`
  }}
/>
        </div>
